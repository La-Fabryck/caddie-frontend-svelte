...
Git hooks ([Lefthook](https://github.com/evilmartians/lefthook), [`lefthook.yml`](lefthook.yml)): pre-commit runs **oxfmt** then **eslint** on staged files (`stage_fixed`), in parallel with **typecheck**. Hooks install via lefthook’s postinstall on `npm ci` / `npm install` (`allowScripts.lefthook`: `true`).
